...
/// 
/// This function processes a single file against the rules and updates the context.
/// It's designed to be called from a parallel iterator to enable concurrent processing
/// of multiple files, which can significantly improve performance, especially for
/// large numbers of files or when file operations are I/O bound.
/// 
/// The function handles thread-safety by using a shared Arc<Mutex<WorkflowContext>>
/// for the context, ensuring that updates to statistics and planned operations are
/// properly synchronized across threads.
/// 
/// # Arguments
/// * `file_info` - Information about the file to process
/// * `config` - The configuration
/// * `context` - The shared workflow context wrapped in Arc<Mutex<>>
/// * `dry_run` - Whether to actually perform file operations (false) or just simulate them (true)
fn process_file_parallel(
    file_info: &crate::discovery::FileInfo,
    ...
    // Update files processed count
    {
        ...
        ctx.increment_files_processed();
    }

    // Check if any of the configuration rules applies
    let matches = match match_file_against_rules(file_info, &config.rules) {
        Ok(m) => m,
        Err(e) => {
            error!("Failed to match file against rules: {e}");
            let mut ctx = match context.lock() {
                Ok(ctx) => ctx,
                Err(e) => {
                    error!("Failed to lock context: {e}");
    ...
    for match_result in matches {
        // Update files matched count
        {
            ...
            ctx.increment_files_matched();
        }

        // If the rule has a defined transformative function, use it to generate the new destination filename
        ...
        let path_result = match generate_destination_path(&transform_result, !dry_run) {
            Ok(result) => result,
            Err(e) => {
        ...
        // If the rule has a defined conversion function, apply it before any other file operations
        let conversion_result = match convert_file_format(&path_result, !dry_run) {
            Ok(result) => {
                if result.source_path != path_result.source_path
                    || result.target_path != path_result.target_path
                {
                    // Update files converted count and add planned operation
                    let mut ctx = match context.lock() {
                        Ok(ctx) => ctx,
                        Err(e) => {
                            error!("Failed to lock context: {e}");
                    ...
                    if dry_run {
                        ...
                        ctx.add_planned_operation(PlannedOperation {
        ...
        // Apply any file operations to the source file (move and/or copy)
        let action_result = match perform_file_action(&conversion_result, !dry_run) {
            Ok(result) => result,
            Err(e) => {
...
///
/// The function uses parallel processing to improve performance by processing multiple files
/// concurrently. This can significantly speed up the processing, especially for large numbers
/// of files or when file operations are I/O bound. Thread safety is ensured by using
...
pub fn process_files(options: ProcessingOptions) -> Result<WorkflowContext> {
    ...
    // Create the workflow context and wrap it in Arc<Mutex<>> for thread-safe access
    ...
    if files.is_empty() {
        ...
        // Unwrap the Arc<Mutex<>> to get the WorkflowContext
        return Ok(Arc::try_unwrap(context)
    ...
    files.par_iter().for_each(|file_info| {
        process_file_parallel(file_info, &config, &context, options.dry_run);
    });
